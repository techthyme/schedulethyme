...
export default function MainHeroV1({ hero }: MainHeroV1Props) {
  ...
  return (
    <>
      ...
      <div className="-mt-32 py-4 bg-white">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-8 w-full">
            ...
            {ctas.map((cta, idx) => (
              <div key={idx} className="md:col-span-1">
                <Link href={`/collabs?filter=${cta.type}`}>
                  <div
                    className={clsx(
                      "relative group overflow-hidden rounded-3xl aspect-[3/4] bg-gray-100",
                      "shadow-lg hover:shadow-xl transition-all duration-500"
                    )}
                  >
                    <Image
                      src={cta.image}
                      alt={`${cta.name} workspace`}
                      fill
